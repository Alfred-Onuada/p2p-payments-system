...
export const verifyTopup = async function (
  ...
  try {
    ...
    const resp = await fetch(`${PAYSTACK_BASE_API}/transaction/verify/${ref}`, {
      method: "GET",
      headers: {
    ...
    await session.withTransaction(async function () {
      try {
        ...
        if (
          ...
          throw {
            name: "PaystackError",
            message: "Could not complete the transaction verification",
          };
        }
    ...
    res
      .status(200)
      .json({
        message: "Topup verified and saved",
        data: userInfo.walletBalance,
      });
  } catch (error) {
...
export const transferFunds = async function (
  req: IExtendedRequest,
  res: Response
) {
  const { userId } = req;
  ...
  try {
    ...
    if (amount.toString() === "NaN" || amount <= 0) {
      res
        .status(400)
        .json({ message: "Please specify a valid amount to transfer" });
      return;
    }
    ...
    if (receiverDoc === null) {
      res
        .status(400)
        .json({ message: "Couldn't find the user specified as recipient" });
      return;
    }
    ...
    const { walletBalance } = await userModel.findOne({
      _id: new mongoose.Types.ObjectId(userId),
    });

    res
      .status(200)
      .json({ message: "Transfer successful", data: { ref, walletBalance } });
  } catch (error) {
    handleAPIError(error, res);
  }
};
